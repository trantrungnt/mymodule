...
//get cmnd from CMND.tpl
$cmnd = $nv_Request->get_title('cmnd','get');

//query by cmnd and Display in update_CMND.tpl
$_sql = "SELECT cmnd, name, birthday, sex, image, thumb, hometown, origin, place, ethnic, religious, date_of_issue, where_licensing, characteristics FROM
".$db_config['prefix'] . "_" . NV_LANG_DATA . "_" . $module_data." WHERE cmnd=:cmnd";
...
/*if (!empty($data['cmnd']) AND !empty($data['name'])
		...
		/*	if( $rows['image'] != '' && file_exists( NV_UPLOADS_REAL_DIR . "/" . $rows['image'] ) )
			{
					...
					/*	try{
							$sql = "UPDATE ".$db_config['prefix'] . "_" . NV_LANG_DATA . "_" . $module_data." SET cmnd=:cmnd, name=:name, birthday=:birthday, sex=:sex, image=:image, thumb=:thumb, hometown=:hometown, origin=:origin, place=:place, ethnic=:ethnic, religious=:religious, date_of_issue=:date_of_issue, where_licensing=:where_licensing, characteristics=:characteristics WHERE cmnd = '".$cmnd."'";
							...
							$query->bindParam(':cmnd', $data['cmnd'], PDO::PARAM_STR, 255);
